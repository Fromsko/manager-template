...
async function login(page: Page) {
  ...
  await expect(page.getByRole('heading', { name: 'Gateway Manager' })).toBeVisible();
...
test.describe('gateway manager e2e', () => {
  ...
  test('supports user management CRUD and filtering', async ({ page }) => {
    ...
    await dialog.getByRole('textbox', { name: /邮箱/ }).fill('playwright@gateway.dev');
